...
template<typename T> struct mcmf {
	...
	vector<T> spfa(int s) { // nao precisa se nao tiver custo negativo
        ...
        for (int i = 0; i < sz(g); ++i) {
            for (auto e : g[i]) if (e.flow < e.cap) deg[e.to]++;
        }
		for (int i = 0; i < sz(g); ++i) if (deg[i] == 0) q.push(i), dist[i] = 0;
		while (!q.empty()) {
			int v = q.front(); q.pop();
			for (int i = 0; i < g[v].size(); i++) {
				auto [to, rev, flow, cap, res, cost] = g[v][i];
				if (res == false) {
					deg[to]--, ckmin(dist[to], dist[v] + cost);
					assert(deg[to] >= 0);
...
void solve(){
    ...
    for (int i = 0; i < n; ++i) {
        for (auto v : g[i]) {
            if (comp[v] != comp[i]) g2[comp[i]].pb(comp[v]);
        }
        val[comp[i]] += tmp[i];
    }
    swap(g, g2);
