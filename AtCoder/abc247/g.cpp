...
template<typename T> struct mcmf {
	...
	vector<T> spfa(int s) { // nao precisa se nao tiver custo negativo
        ...
        for (int i = 0; i < g.size(); ++i) for (auto j : g[i]) if (!j.res) deg[j.to]++;
		dist[s] = 0;
		q.push(s);
		while (!q.empty()) {
	...
	void solve(int s, int t, int flow = INF) {
...
void solve(){
    ...
    mcmf<int> fluxo(302);
    int src = 300, sink = 301;
    for (int i = 0; i < 150; ++i) {
        fluxo.add(src, i, 1, 0);
        fluxo.add(i+150, sink, 1, 0);
    }
    for (int i = 0; i < n; ++i) fluxo.add(a[i], b[i]+150, 1, -c[i]);
    fluxo.solve(src, sink);
}
