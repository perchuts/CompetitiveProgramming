...
void solve(){
    ...
    // minimize \sum x[i] * a[i] + y[i] * b[i]
    // subject to
    // x[i], y[i] >= 0
    // x[i] + y[j] >= c[i][j] (k[i][j])
    // Dual: variables k[i][j]
    // constraints:
    // k[i][j] >= 0
    // pro x[i]: \sum{i} k[i][j] <= a[i]
    // pro y[j]: \sum{j} k[i][j] <= b[j]
    // maximize \sum k[i][j] * c[i][j]
    // mcmf: vertices sao os lados da biparticao. source 
    mcmf<int> f(l+r+2);
    int src = l+r, sink = src+1;
    for (int i = 0; i < l; ++i) f.add(src, i, a[i], 0);
    for (int i = 0; i < r; ++i) f.add(l+i, sink, b[i], 0);
    for (int i = 0; i < l; ++i) {
        for (int j = 0; j < r; ++j) {
            f.add(i, l+j, inf, -c[i][j]);
        }
    }
    auto [x, y] = f.min_cost_flow(src, sink);
    cout << -y << endl;
}
