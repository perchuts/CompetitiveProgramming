...
int solve(int n, vector<vector<int>> g) {
    ...
    auto dfs = [&] (auto&& self, int u, int p) -> void {
        ...
        // dp[0]
        // 2 * dp[a][2] * dp[b][2] * \prod (dp[c][0] + 2*dp[c][2])
        {
            vector<int> pref(sz(cc)+1, 1), suf = pref;
            for (int i = 1; i <= sz(cc); ++i) pref[i] = pref[i-1] * (dp[cc[i-1]][0] + 2 * dp[cc[i-1]][2]) % mod;
            for (int i = sz(cc)-1; i >= 0; --i) suf[i] = suf[i+1] * (dp[cc[i]][0] + 2 * dp[cc[i]][2]) % mod;
            int add = 0, cum = 0;
        ...
        // dp[0]
        {
            ...
            vector<int> pref(sz(cc)+1, 1), suf = pref;
            for (int i = 1; i <= sz(cc); ++i) pref[i] = pref[i-1] * dp[cc[i-1]][0] % mod;
            for (int i = sz(cc)-1; i >= 0; --i) suf[i] = suf[i+1] * dp[cc[i]][0] % mod;
            int um = 0;
            for (int i = 0; i < sz(cc); ++i) {
                dp[u][0] = (dp[u][0] + 2 * um * dp[cc[i]][1] % mod * suf[i+1]) % mod;
