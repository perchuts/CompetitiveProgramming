...
void solve(){
    int n; cin >> n;
    vector<mint> fat(n+1, 1), ifat(n+1), bicho(n+1, 1), dp(n+1);
    ...
    int B = 1000;
    int sexo = n * (mod+1) / 2 % mod;
    for (int i = 1; i <= n; ++i) bicho[i] = bicho[i-1] * sexo;
    ifat[n] = mint(1)/fat[n];
    for (int i = n-1; ~i; --i) ifat[i] = (i+1) * ifat[i+1];
    dp[0] = fexp(n*n%mod, mod-2) * fat[n];
    vector<mint> P = {fexp(n*n%mod, mod-2) * fat[n]};
    for (int i = 3; i <= n; ++i) {
        if (!primo[i]) continue;
        if (i <= B) {
            vector<mint> Q(n+1);
            for (int j = 0; j * i <= n; ++j) Q[i*j] = ifat[j] * bicho[j];
            P = convolution(P, Q);
            P.resize(n+1);
        } else {
            continue;
            for (int j = n; j >= i; --j) {
                for (int use = 1; use * i <= j; ++use) {
                    P[j] += P[j - use * i] * ifat[use] * bicho[use];
                }
            }
        }
    }
    mint ans = 0;
    for (int i = 0; i <= n; ++i) ans += P[i] * ifat[n-i] * fexp(n, n-i);
    cout << ans << endl;
}
