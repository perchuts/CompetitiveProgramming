...
void solve(){
    ...
    vector<int> d(k);
    for (auto& x : d) cin >> x;
    vector<mint> P(n), fat(n, 1), ans(n);
    ans[0] = 1;
    for (int i = 1; i < n; ++i) fat[i] = i * fat[i-1];
    for (auto x : d) P[x-1] = mint(1)/fat[x-1];
    int e = n;
    while (e) {
        if (e&1) ans = convolution(ans, P), ans.resize(n);
        e /= 2, P = convolution(P, P), P.resize(n);
    }
    mint resp = ans[n-2] * fat[n-2];
    cout << resp.v << endl;
}
