...
void solve(){
    ...
    auto dnc = [&] (auto&& self, int l, int r) -> vector<mint> {
        if (l == r) {
            vector<mint> eu = {1, a[l]};
            return eu;
        }
        int md = (l + r) / 2;
        auto p1 = self(self, l, md);
        auto p2 = self(self, md+1, r);
        return convolution(p1, p2);
    };
    auto P = dnc(dnc, 0, n-1);
    P.pb(mint(0));
    vector<mint> fat(n+1, 1);
    for (int i = 1; i <= n; ++i) fat[i] = fat[i-1] * i, P[i] *= fat[i];
    mint prob = 1, ans = 0;
    for (int i = 1; i <= n; ++i) {
        // o espaco amostral eh tudo com ate i+1 formas de fazer!
        // quero atualizar a probabilidade pro proximo passo
        //
        mint morrer = prob * (P[i] * mint(sum-i) - P[i+1]) / (P[i] * mint(sum-i)); 
        ans += morrer * (i+1);
        prob -= morrer;
    }
    cout << ans << endl;
