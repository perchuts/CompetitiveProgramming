...
void solve(){
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        int prv = 0;
        vector<ii> intrs;
        for (int j = 0, k = 0; j < sz(sexo[i]); j = k) {
            while (k != sz(sexo[i]) and sexo[i][k].first == sexo[i][j].first) {
                if (sexo[i][k].second != sexo[i][j].second) {
                    cout << "NO" << endl;
                    exit(0);
                }
                k++;
            }
            intrs.pb({sexo[i][j].first, sexo[i][j].second-prv});
            prv = sexo[i][j].second;
        }
        nr.pb({i, intrs[0].first, intrs[0].second});
        for (int i = 1; i < sz(intrs); ++i) sexo[intrs[i-1].first+1].pb(intrs[i]);
    }
    l.clear(), r.clear(), s.clear();
    x = sz(nr);
    for (auto [a, b, c] : nr) l.pb(a), r.pb(b), s.pb(c);
