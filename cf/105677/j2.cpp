...
void solve(){
    ...
    vector<array<int, 4>> z(s);
    vector<vector<ii>> vals(s);
    int help = 0;
    for (auto& v : z) {
        char c; cin >> c;
        v[0] = (c == 'V');
    ...
    for (int i = 0; i < s; ++i) {
        auto v = z[i];
        if (v[0]) { 
            for (int j = v[1]+1; j < n and grid[j][v[2]] > 0; ++j) vals[i].pb({j, v[2]});
        } else {
            for (int j = v[2]+1; j < m and grid[v[1]][j] > 0; ++j) vals[i].pb({v[1], j});
        }
        z[i][3] -= (int)vals[i].size();
    }
    int sum = 0;
    for (int i = 0; i < n; ++i) for (int j = 0; j < m; ++j) grid[i][j]--, sum += max(0, grid[i][j]);
    int src = s+2*n*m, sink = src+1;
    mcmf<int> fluxo(s+2*n*m+2);
    const int INF = 100000;
    for (int i = 0; i < s; ++i) {
        auto v = z[i];
        if (!v[0]) {
            help += z[i][3];
            fluxo.add(src, 2*n*m+i, v[3], 0);
            for (auto [x, y] : vals[i]) fluxo.add(2*n*m+i, m*x+y, grid[x][y], -1), fluxo.add(2*n*m+i, m*x+y, 8-grid[x][y], 1);
        }
        else {
            fluxo.add(2*n*m+i, sink, v[3], 0);
            for (auto [x, y] : vals[i]) fluxo.add(m*x+y+n*m, 2*n*m+i, INF, 0);
        }
    }
    for (int i = 0; i < n; ++i) for (int j = 0; j < m; ++j) {
        if (grid[i][j] == -1) continue;
        fluxo.add(m*i+j, n*m+m*i+j, 8, 0);
    }
    auto [fl, cost] = fluxo.min_cost_flow(src, sink, INF);
    if (fl == help) cout << cost+sum << endl;
